...
class LibraryTest(TestCase):
    def setUp(self):
        test_user1 = User.objects.create_user(username='koi',email='koryun9999@mail.ru',password='11091998k',first_name='Koryun',last_name='Marabyan')
        test_user2 = User.objects.create_user(username='ivan',email='ivan@mail.ru',password='ivan1234',first_name='Ivan',last_name='Seliverstov')
        test_user1.save()
        test_user2.save()
...
#search_book		
    ...
    def test_search_book_logged_in_correct_get_request(self):
        login = self.client.login(username='koi', password='11091998k')
        resp = self.client.get('/library/search_book/')
        self.assertEqual(str(resp.context['user']), 'koi')
...
#main	
    ...
    # def test_registration_newUser(self):
    #     resp = self.client.get('/library/registration/')
    #     resp = self.client.post(reverse('registration'), {'username': 'Koryun', 'password': '11091998k', 'email': 'marabyankoi@gmail.com', 'first_name': 'Koryun', 'last_name': 'Marabyan'})
    #     self.assertEqual(resp.status_code, 200)
